...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	//select MAX3421E
	SpiInstance.SlaveSelectReg = 0; //unsure
	//write reg + 2 via SPI
	BYTE data[2];
	int ret_code;
	data[0] = reg + 2;
	data[1] = val;
	ret_code = XSpi_Transfer(&SpiInstance, data, NULL, 2);
	if (ret_code != 0) {
		xil_printf("Error!");
	}
}
...
BYTE MAXreg_rd(BYTE reg) {
	...
	int ret_code;
	data[0] = reg;
	ret_code = XSpi_Transfer(&SpiInstance, data, recv, 2);
	if (ret_code != 0) {
		xil_printf("Error!");
	}
	return recv[1];
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	ret_code = XSpi_Transfer(&SpiInstance, send, NULL, nbytes+1);
	if (ret_code != 0) {
		xil_printf("Error!");
	}
	return (data + nbytes);
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	BYTE recv[nbytes+1];
	send[0] = reg;
	//ret_code_0 = XSpi_Transfer(&SpiInstance, send, NULL, 1);
	ret_code = XSpi_Transfer(&SpiInstance, send, recv, nbytes+1);
	int i;
	for (i = 0; i < nbytes; i++) {
		data[i] = recv[i+1];
	}
